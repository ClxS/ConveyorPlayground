...
void cpp_conv::ui::drawUI(SceneContext& kSceneContext, const RenderContext& kRenderContext)
{
    ...
    {
        ImGui::Text("%.3f ms/frame (%.1f FPS)", static_cast<double>(1000.0f / ImGui::GetIO().Framerate), static_cast<double>(ImGui::GetIO().Framerate));
        ImGui::Text(std::format("Entities in world: {} Conveyors, {} Others", kSceneContext.m_rMap.GetConveyors().size(), kSceneContext.m_rMap.GetOtherEntities().size()).c_str());
