...
namespace
{
    ...
    void debugDraw(const cpp_conv::components::SquareSolarBodyComponent& solarBody, const Eigen::Vector3f& sphericalLookAtLocation)
    {
        using namespace atlas::render::debug;
        debug_draw::createScope();
        debug_draw::setWireframe(true);

        const auto& points = solarBody.m_SphereData.m_Polyhedron.GetPoints();

        debug_draw::setColor(0xaa000000);
        constexpr bool c_showCells = true;
        if constexpr (c_showCells)
        {
            for(const auto& square : solarBody.m_SphereData.m_Polyhedron.GetSquares())
            {
            ...
            if constexpr (false)
            {
                for(const auto& triangle : solarBody.m_SphereData.m_Polyhedron.GetTriangles())
                {
    ...
    void debugDraw(const cpp_conv::components::HexagonSolarBodyComponent& solarBody, Eigen::Vector3f& matrix)
    {
...
void cpp_conv::SolarBodyRenderSystem::Update(atlas::scene::EcsManager& ecs)
{
    ...
    for(auto [entity, model, position] : ecs.IterateEntityComponents<components::ModelComponent, atlas::game::scene::components::PositionComponent>())
    {
    ...
    for(auto [entity, camera] : ecs.IterateEntityComponents<atlas::game::scene::components::cameras::SphericalLookAtCameraComponent_Private>())
    {
    ...
    for(auto [entity, solarBody] : ecs.IterateEntityComponents<components::SquareSolarBodyComponent>())
    {
        bgfx::setState(0
        ...
        debugDraw(solarBody, sphericalLookAtLocation);
    }

    for(auto [entity, solarBody] : ecs.IterateEntityComponents<components::HexagonSolarBodyComponent>())
    {
        bgfx::setState(0
                        | BGFX_STATE_WRITE_RGB
                        | BGFX_STATE_WRITE_A
