...
#include "ImguiEmbeddedShaders.h"
...
void ImGui_Implbgfx_RenderDrawLists(ImDrawData* draw_data)
{
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++) {
        ...
        const auto verts = reinterpret_cast<ImDrawVert*>(tvb.data);
        memcpy(
            verts, cmd_list->VtxBuffer.begin(),
            numVertices * sizeof(ImDrawVert));

        const auto indices = reinterpret_cast<ImDrawIdx*>(tib.data);
        memcpy(
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback) {
                ...
                const bgfx::TextureHandle texture = {
                    static_cast<uint16_t>(reinterpret_cast<intptr_t>(pcmd->TextureId) & 0xffff)};
